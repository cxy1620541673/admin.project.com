...
class Task extends PW_Controller {
	...
	public function AjaxGetTaskList() {
		...
		$field				=	't_id,t_title,t_executor_id,t_start_time,t_end_time,t_status,t_create_time';
		$data['rows']		=	$this->taskmodel->GetAllTask( $field, $condition );
		foreach ($data['rows'] as &$v) {
			$v['t_status']		=	$status[$v['t_status']].' [ <span data-id="'.$v['t_id'].'"><a class="edit-status" href="javascript:;" data-val="2">开始</a> | <a class="edit-status" href="javascript:;" data-val="3">完成</a> | <a class="edit-status" href="javascript:;" data-val="4">上线</a></span> | <a class="edit-status" href="javascript:;" data-val="5">放弃</a></span> ] ';
	...
	public function AjaxUpdateTaskStatus() {
		...
		if ( $task_info['t_status'] != 5 && $task_info['t_status'] > $data['t_status'] )	
			EchoJson( FALSE, OPERATE_FALSE, '阶段已完成，状态未发生改变' );
		if ( $data['t_status'] == $task_info['t_status'] )	
